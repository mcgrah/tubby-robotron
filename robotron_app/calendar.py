...
def generate_month_manual(year_number=(datetime.now().year), month_number=(datetime.now().month)):
    # start_day = datetime(year_number, month_number, 1).weekday()
    ...
    month_text = ''
    month_text += '<table class="table table-hover" id="calendar_month"><thead class="thead-light">'+'\n'
    ...
    for day_title in day_titles:
        month_text+='<th class="day_title">'+day_title+'</th>'+'\n'
    ...
    for i in range(start_day+1,7):
        ...
        # else:
        month_text += '<td class="day">' + str(day) + '</td>' + '\n'
        day += 1
